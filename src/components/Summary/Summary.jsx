            ...
            {packages.length > 0 && <div className="summary-container">
                ...
                {packages.map((el, index) => el.productType !== '' && <div key={index} className='summary__elem-wrapper'>
                    <div className="box">
                        ...
                        {cost && <div className="summary__elem-cost"><p>{cost[index] && '$' + costs.package / 100 + ' X ' + el.volume}</p>{el.answerTime !== "" && <span>{el.answerTime} h</span>}</div>}
                        {console.log(costs.package, 'цена не делённая на 100')}
                    </div>
